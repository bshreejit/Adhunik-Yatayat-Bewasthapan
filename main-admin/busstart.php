...
<body>
   ...
   // to retrieve data from table
    $result= mysqli_query($conn, "SELECT * FROM bus WHERE status=0 AND id = $view_id");
  }
          ...
          <table class="table">
            ...
            <?php 
              while ($res = mysqli_fetch_assoc($result)) {?>
                ...
                <button type="button" class="btn btn-lg btn-danger" type="decline" name="decline"><a onclick="return confirm('Are you sure to send this bus?');" href="send.php?id=<?= $view_id?>&table=route" title="Delete">Send</a></button>
            </div>
            <?php }?></td>
